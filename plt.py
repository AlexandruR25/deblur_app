...
def automated_trainer():
    """Antrenare automată cu reluare și salvare progres + Hard Example Mining"""
    ...
    criterion = nn.L1Loss(reduction='none')  # schimbăm în 'none' pentru HEM
    best_loss = float('inf')
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        for batch_idx, (blur, sharp) in enumerate(train_loader):
            ...
            # Calculăm loss individual pe imagine
            loss_tensor = criterion(outputs, sharp).mean(dim=(1, 2, 3))  # [batch_size]
            k = max(1, int(0.5 * len(loss_tensor)))  # luăm 50% imagini cu cel mai mare loss

            # Selectăm top-k exemple cu loss mare
            _, indices = torch.topk(loss_tensor, k=k)
            selected_blur = blur[indices]
            selected_sharp = sharp[indices]

            # Forward și backward doar pe exemplele selectate
            selected_outputs = model(selected_blur)
            final_loss = nn.L1Loss()(selected_outputs, selected_sharp)  # L1 standard
            final_loss.backward()
            optimizer.step()

            total_loss += final_loss.item()
            images_processed += selected_blur.size(0)
